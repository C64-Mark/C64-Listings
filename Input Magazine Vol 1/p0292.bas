10 dim w$(21),wo$(33):for z=1 to 33:read wo$(z):next
20 for z=1 to 40:li$=li$+"-":next:li$="{white}"+li$
30 print"{clear}{yellow}"tab(7)"******typ{orange}ing{yellow}tut{orange}or*****"
40 a$="asdfghjkl:":poke 54296,15:goto 380
50 ti$="000000":s=0:ww=1
60 n=0:poke 198,0 
70 if k=5 and p>0 then n=n+1:p=p+1:goto 140
80 print"{home}{down*5}";:if k<3 then print tab(11)"{cyan}{reverse on}a s d f g h j k l :":poke 198,0
90 x=int(rnd(1)*10)+1:n=n+1:p=p+1:if k=1 then x=n:goto 120
100 if k=3 then x=int(rnd(1)*10)+1:print tab(18)"{cyan}U{sh asterisk}I{down}{left*3}{sh -}{green}"mid$(a$,x,1)"{cyan}{sh -}{down}{left*3}J{sh asterisk}K"
110 if k=4 then print tab(16)"{cyan}{space*5}{left*5}"w$(ww):x=n*.5+3
120 if k<3 or k=4 then print"{home}{down*4}"tab(11+(x-1)*2)"{white}*"
130 if k=5 then for z=1 to 7:print"{yellow}"w$(z);:next z:print:print"{reverse on}{cyan}";
...
210 w=54276:a=54277
220 if k<3 or k=4 then print tab(11+(x-1)*2)"{up} "
...
350 nu=0:p=0:s=0:for ww=1 to mm:w$(ww)=wo$(int(rnd(1)*33)+1)
360 if k=5 and ww<>7 then w$(ww)=w$(ww)+" "
...
470 if k=5 then print tab(12)"{up}type these words":mm=7
...
540 data ask,sad,dad,glad,had,flash,flask,lash,slash,lag,has,haj,ska,has,hash
550 data dash,ass,shall,ash,all,fall,hag,gash,flag,jag,gal,lass,fag,gas,gaff
