...
40 a$="qazwsxedcrfvtgbyhnujmik,ol.p;/":poke 54296,15:goto 380
50 ti$="000000":s=0:ww=1
60 n=0:poke 198,0 
70 if k=5 and p>0 then n=n+1:p=p+1:goto 140
80 print"{home}{down*5}";:if k<3 then printtab(5)"{cyan}{reverse on}"a$:poke 198,0
90 x=int(rnd(1)*30)+1:n=n+1:p=p+1:if k=1 then x=n:goto 120
100 if k=3 then x=int(rnd(1)*30)+1:print tab(18)"{cyan}U{sh asterisk}I{down}{left*3}{sh -}{green}"mid$(a$,x,1)"{cyan}{sh -}{down}{left*3}J{sh asterisk}K"
110 if k=4 then print tab(16)"{cyan}{space*10}{left*10}"w$(ww):x=n+11
120 if k<3 or k=4 then print"{home}{down*4}"tab(5+(x-1))"{white}*"
...
210 w=54276:a=54277
220 if k<3 or k=4 then print"{home}{down*4}"tab(5+(x-1))" "
...
540 data typing,through,think,viewed,setting,pack,crave,meant,limbs,tickle
550 data longer,swarm,want,mate,murder,wind,quiz
560 data window,car,nothing,caravan,name,slave,border,zero,bread,mode,save
