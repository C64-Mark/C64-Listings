...
190 if gs(1,1)=0 and gs(2,1)=0 and gs(3,1)=0 and gs(4,1)=0 thenreturn
...
197 if gs(fg,1)=pgthenpoke 49154,21:goto 200
198 k=gs(fg,1):if k=3ork=19ork=36thenpoke 49154,22:sys(49157):goto 500
199 poke 49154,20
200 sys(49157)
202 x=peek(197):if x=42orx=50thengosub 100
206 poke 49202,32:poke 49217,0:poke 49153,gs(fg,2)+1
208 sys(49157)
210 poke 54276,0:return
220 if gs(1,1)<>0then225
221 i=int(rnd(1)*39+1):if i=gs(2,1)ori=gs(3,1)ori=gs(4,1)then221
222 gs(1,1)=i:gs(1,2)=0:poke 1024+i,42:poke 55296+i,7:return
225 if gs(2,1)<>0then230
226 i=int(rnd(1)*39+1):if i=gs(1,1)ori=gs(3,1)ori=gs(4,1)then226
227 gs(2,1)=i:gs(2,2)=0:poke 1024+i,42:poke 55296+i,7:return
230 if gs(3,1)<>0then235
231 i=int(rnd(1)*39+1):if i=gs(1,1)ori=gs(2,1)ori=gs(4,1)then231
232 gs(3,1)=i:gs(3,2)=0:poke 1024+i,42:poke 55296+i,7:return
235 if gs(4,1)<>0then240
236 i=int(rnd(1)*39+1):if i=gs(1,1)ori=gs(2,1)ori=gs(3,1)then236
237 gs(4,1)=i:gs(4,2)=0:poke 1024+i,42:poke 55296+i,7:return
240 if gs(1,3)=1andgs(2,3)=1andgs(3,3)=1andgs(4,3)=1thenph=1:return
242 i=int(rnd(1)*4+1):if gs(i,3)=1then242
243 poke gs(i,1)+40*gs(i,2)+1024,32:gs(i,2)=gs(i,2)+1
244 poke gs(i,1)+40*gs(i,2)+1024,42:poke gs(i,1)+40*gs(i,2)+55296,7
246 if rnd(1)<.3orgs(i,2)>=10thengs(i,3)=1
248 return
250 if peek(v+21)<>0then255
252 if rnd(1)<.95thenreturn
253 mc=2:poke v,25:poke v+1,50:poke v+21,1:poke v+16,0:return
255 if peek(v)=253thenpoke v,0:poke v+16,1:goto 260
257 poke v,peek(v)+3
260 if peek(v)>=50andpeek(v+16)=1thenpoke v+21,0:return
263 i=peek(v)+256*(peek(v+16)and1):mc=int((i-23)/8)+3
265 if rnd(1)<.55thenreturn
270 if mc<>gs(1,1)andmc<>gs(2,1)andmc<>gs(3,1)andmc<>gs(4,1)thengosub 280
275 return
...
284 if peek(1784+mc)=102thenpoke 49154,20:goto 290
287 if mc=pgthenpoke 49154,21:goto 290
288 if mc=3ormc=19ormc=36thenpoke 49154,22:sys(49157):goto 500
289 poke 49154,20
290 sys(49157)
292 x=peek(197):if x=42orx=50thengosub 100
...
360 return
400 if gs(1,1)<>0orgs(2,1)<>0orgs(3,1)<>0orgs(4,1)<>0then405
402 fori=1to4:gs(i,3)=0:next:ph=0:return
405 i=int(rnd(1)*4+1):if gs(i,1)=0then405
407 poke 1024+gs(i,1)+gs(i,2)*40,32:if gs(i,2)>0thengs(i,2)=gs(i,2)-1
410 poke 1024+gs(i,1)+gs(i,2)*40,42:poke gs(i,1)+gs(i,2)*40+55296,7
412 if gs(i,2)=0thenpoke 1024+gs(i,1),32:gs(i,1)=0
415 return
500 if peek(1824+pg)=113then560
502 poke v+40,0:poke v+28,2
504 poke 2041,193:k=pg*8+15:if k>255thenpoke v+2,k-256:poke v+16,2:goto 506
505 poke v+16,peek(v+16)and253:poke v+2,k
506 poke v+3,200:poke v+21,3:gosub 700
508 poke 54276,0:poke 54277,0
509 fort=1to1000:next:poke 2040,192:poke v+21,0:poke v+39,3:poke 54296,0
510 poke 53280,6:poke 53281,13:print"{clear}{black}{space*14}laser battle":print
515 nm=nm-1:if nm=0then600
520 print"you have ";nm;" men left":print
530 fort=1to3000:next
535 fori=1to4:gs(i,1)=0:next:ph=0
550 goto 50
560 fori=1to3:poke 53280,int(rnd(1)*10)+23:gosub 700:next:poke v+21,0
...
630 getx$:if x$=""thenend
640 goto 630
700 forx=15to0step-1:poke 54296,x:poke 54276,129
